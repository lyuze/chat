package com.lyz.chat.service.impl;

import com.lyz.chat.handler.WebSocketServerHandler;
import com.lyz.chat.service.MyHttpService;
import com.lyz.chat.service.MyWebSocketService;
...
//                    System.out.println("channel close future  {}",channelFuture);
        ...
        try {
            ...
            channelFuture.channel().closeFuture().addListener((ChannelFutureListener) channelFuture12 ->
                    System.out.println("server channel {, channelFuture12.channel()} closed.")).sync();
    ...
    public void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest request) {
        ...
        if (isWebSocketUpgrade(request)){
            //如果是webSocket请求
            System.out.println("请求是webSocket协议");
            // 获取子协议
            ...
            // 不处理 HTTP 请求
            System.out.println("不处理 HTTP 请求");
        }
    }
...
//            System.out.println("receive textWebSocketFrame from channel: {} , 目前一共有{}个在线",ctx.channel(),channelMap.size());
            //发给其它的 channel  (群聊功能)
...
//                System.out.println("消息已发送给{}",ch);
//                System.out.println("write text: {} to channel: {}",textWebSocketFrame,ctx.channel());
            }
            return;
        }
        /**
...
//            System.out.println("receive pingWebSocket from channel: {}",ctx.channel());
...
//            System.out.println("receive pongWebSocket from channel: {}",ctx.channel());
            return;
        }
        /**
         * close frame, close
        ...
        if (frame instanceof CloseWebSocketFrame){
            System.out.println("receive closeWebSocketFrame from channel: {, ctx.channel()}");
            //获取到握手信息
            WebSocketServerHandshaker handshaker = ctx.channel().attr(ATTR_HAND_SHAKER).get();
            if (handshaker == null){
                System.err.println("channel: {, ctx.channel()} has no handShaker");
                return;
            }
         ...
         * 剩下的都是 二进制 frame ，忽略
         */
        System.out.println("receive binary frame , ignore to handle");
    }
